...
import e.administrator.xy.util.qiniuUtil.Json;

/**
 * A simple {@link Fragment} subclass.
 */
public class activityListUnderLine extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View layout =inflater.inflate(R.layout.fragment_activity_list_under_line,null);
        init(layout);
        initRefreshLayout();
        initData();
        return layout;
    ...
    private void initData() {
        ...
        if (classify.equals("examineActivity")||classify.equals("falseExamineActivity")){
                    ...
                    if (json!=null){
                        try {
                            ...
                            if (!sortActivity.isEmpty()){
                                ...
                                Toast.makeText(getContext(), "线下活动为空哟", Toast.LENGTH_SHORT).show();
                            }
                        }catch (Exception e) {
                            e.printStackTrace();
                        }
            ...
            client.post(constant.BASE_URL + constant.activity_getUnderLine, new AsyncHttpResponseHandler() {
                @Override
                public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                    String json = new String(responseBody);
                    if (json!=null){
    ...
    private void init(View layout) {
        refreshLayout = layout.findViewById(R.id.refresh_ActivityListUnderline);
    }
}
